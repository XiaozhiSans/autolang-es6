/**
 * autolang-es6 v1.4.4-es6 by @XiaozhiSans
...
export const al = {
	load(l = navigator.language, mode = this.mode.HTML, callback = () => {}) {
		let getApplies = (e, applyTo) => {
		...
		for (var i = 0; i < text.length; i++) {
			...
			al.lang[l][text[i]] = p.replace(/(?<!\\)((?:\\\\)*)\$\{([^}]*)}/g, (m, s, expr) => {
		...
		for (var i = 0; i < text.length; i++) {
			document.querySelectorAll('[al="' + text[i] + '"]').forEach((e) => {
				let applyTo = e.getAttribute('al-aplto')
				if (applyTo !== null && applyTo !== void 0) {
					let applies = getApplies(e, applyTo)
